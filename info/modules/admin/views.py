...
def user_count():
    ...
    try:
        ...
        current_app.logger.error(e)
        return jsonify(errno=RET.DBERR,errmsg='查询用户总人数失败')
    # 定义月新增人数：每月1日的0时0分时间以后，新注册的用户
    mon_count = 0
    # 必须要拿到时间对象，拼接字符串，获取每月的开始日期
    # tm_year=2018, tm_mon=11, tm_mday=1
    t = time.localtime()
    # 定义每月的开始日期的字符串,标准日期格式：2018-11-01、2019-01-01
    mon_begin_date_str = '%d-%02d-01' % (t.tm_year,t.tm_mon)
    # 把日期字符串转成日期对象，%Y-%m-%d表示标准日期格式
    ...
    # 结束时间：2018-11-02-00-00-00
    # 使用循环语句，让日期减去一个月的时间,获取每天的开始时间和结束时间
    for d in range(31):
        begin_time = now_date - timedelta(days=d)
        end_time = now_date - timedelta(days=(d-1))
        # 查询数据库，比较每天的日期
        try:
        ...
        active_date.append(begin_time_str)
